...
def flip_image_horizontal(filename, directory):
    ...
    final_path = directory + '\\Augmented\\' + \
        filename[:filename.index('.')] + '_Horizontal_Flip.png'
...
def flip_image_vertical(filename, directory):
    ...
    final_path = directory + '\\Augmented\\' + \
        filename[:filename.index('.')] + '_Vertical_Flip.png'
...
def rotate_image(filename, directory):
    ...
    final_path = directory + '\\Augmented\\' + \
        filename[:filename.index('.')] + '_Rotated_' + str(degrees) + '.png'
...
def zoom_image(filename, directory):
    ...
    final_path = directory + '\\Augmented\\' + \
        filename[:filename.index('.')] + '_Zoomed_' + \
        str(int((zoom_per-1)*100)) + '.png'
...
def adjust_img_saturation(filename, directory):
    ...
    final_path = directory + '\\Augmented\\' + \
        filename[:filename.index('.')] + '_Saturated_' + \
        str(saturation_per) + '.png'
    keras.utils.save_img(final_path, saturated_img)
...
def adjust_img_hue(filename, directory):
    ...
    final_path = directory + '\\Augmented\\' + \
        filename[:filename.index('.')] + '_Hue_' + str(hue_per) + '.png'
    keras.utils.save_img(final_path, hue_img)
...
def adjust_img_contrast(filename, directory):
    ...
    final_path = directory + '\\Augmented\\' + \
        filename[:filename.index('.')] + '_Contrast_' + \
        str(contrast_per) + '.png'
    keras.utils.save_img(final_path, contrast_img)
# @title augment


def augment_data(directory, count_to_match):
    starting_val = len(os.listdir(directory + 'Augmented\\')
                       ) + len(os.listdir(directory)) - 1
    ...
    if (len(os.listdir(directory))-1) + len(os.listdir(directory + "Augmented\\")) == count_to_match:
        print(
            f"Count verified, total of {count_to_match} images with {len(os.listdir(directory))-1} original and {len(os.listdir(subfolder))} augmented.")
    else:
        print(
            f"Incorrect amount. Meant to have {count_to_match}, found {(len(os.listdir(directory))-1) + len(os.listdir(subfolder))} .")
    ...
    for filename in os.listdir(target_dir):
        if filename[30] == '_':
            ...
            image = Image.fromarray(
                (np.array(Image.open(target_dir + filename)) >> 8).astype(np.uint8)).convert('RGB')
